...
namespace RPISVR_Managements.Student_Score.Insert_Student_Score
{
    public sealed partial class Insert_Student_Score_Info : Page
    {

        public StudentViewModel ViewModel { get; set; }
        private DatabaseConnection _ConnectionString;

        public Insert_Student_Score_Info()
        {
        ...
        private void SelectionChanged_Student_Score(object sender, SelectionChangedEventArgs e)
        {
            var listview = sender as ListView;
            var selectedStudent_Score_List = listview.SelectedItems.Cast<Class_Score>().ToList();
            var first_selectedStudent_Score = listview.SelectedItems.Cast<Class_Score>().FirstOrDefault();

            if (DataContext is StudentViewModel viewModel_Student_Score)
            {
                
                viewModel_Student_Score.Multi_Selected_Student_Score = selectedStudent_Score_List;
                viewModel_Student_Score.First_Selected_Student_Score = first_selectedStudent_Score;
            }
        }
        ...
        private void btn_select_all(object sender, RoutedEventArgs e)
        {
            if (sender is ToggleButton toggleButton)
            {
                ...
                if (Table_StudentAndScore_Info_List.ItemsSource is IEnumerable<object> items)
                {
                    if (toggleButton.IsChecked == true) // If ToggleButton is checked
                    {
                        // Select all items
                        Table_StudentAndScore_Info_List.SelectedItems.Clear();
                        foreach (var item in items)
                        {
                            Table_StudentAndScore_Info_List.SelectedItems.Add(item);
                        }       
                    }
